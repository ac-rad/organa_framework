...
def parse_hardware(root, error_list, available_hardware):
    ...
    found_hardware, found_component= False, False
    for item in tags[1:]: # ignore first tag, it's just HARDWARE
        #if item not in ["Hardware", "Component"]:
        if item not in ["Component"]:
            error = "The Hardware section should only contain Component tags"
    #if not found_hardware or not found_component:
    #    error += " The Hardware section must contain Hardware and Component tags"
        ...
        for component in hardware.iter('Component'):
            if available_hardware:
                if component.attrib['id'] not in available_hardware:
                    ...
                    #print("!!!!!!! WRONG HARDWARE", wrong_hardware)
                    error_str = f"{wrong_hardware} is not defined in the given Hardware list. The available Hardware is: {', '.join(available_hardware)}."
        ...
        for reagent in reagents.iter('Reagent'):
            if available_reagents:
                if reagent.attrib['name'] not in available_reagents:
                    ...
                    error_str = f"{wrong_reagent} is not defined in the given Reagents list. The available reagents are: {', '.join(available_reagents)}."
            ...
            if action not in mandatory_properties:
                ...
                for attr in step.attrib:
                    if attr not in optional_properties[action]:
                        ...
                        error_base = f"The {attr} property in the {action} procedure is not allowed."
                        if len(allowed_actions) > 0:
                            error_base += f" The allowed properties are: {', '.join(allowed_actions)}."
                        else:
                            error_base += " This action has no properties."
...
def verify_synthesis(root, available_hardware, available_reagents):
    ...
    error_list = verify_procedure(root, hardware, reagents, error_list) + error_list
    return error_list
