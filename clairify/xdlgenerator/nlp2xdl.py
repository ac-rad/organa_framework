...
sys.path.append(os.path.join(wd, "clairify/"))
sys.path.append(os.path.join(wd, "clairify/xdlgenerator"))
...
def generate_xdl(file_path, available_hardware=None, available_reagents=None):
    ...
    if os.path.exists(file_path):
        ...
        instructions = file_path ### passed in plan directly
    XDL = open("./clairify/XDL_description.txt", "r").read()
    ...
    if available_hardware!= None:
        hardware_str = ", ".join(available_hardware)
        constraints = f"\nThe available Hardware is: {hardware_str}\n"
    if available_reagents!= None:
        reagents_str = ", ".join(available_reagents)
